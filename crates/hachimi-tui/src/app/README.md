...
| `event.rs` | 终端事件分发（`handle_event`）；Normal/Search/Login 三种输入模式的键盘处理；`handle_message` 处理所有 `AppMessage` |
| `actions.rs` | 业务动作：认证流程（captcha → login → logout）、数据加载、播放控制（play/pause/next/prev/seek）、Miller Columns 导航、图片异步加载 |
